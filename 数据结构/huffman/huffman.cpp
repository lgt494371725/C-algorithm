...
htTree * buildTree(char *inputString)
{
	...
	//The queue which will hold the tree nodes，最优队列 
	pQueue * huffmanQueue;//头指针 ，huffmanQueue是地址 
	initPQueue(&huffmanQueue);//存放地址的地址 
	...
	for(int i=0; i<256; i++)
		if(probability[i]!=0)
		{
			...
			aux->left = NULL;
			aux->right = NULL;
			aux->symbol = (char) i;//将ASDII码转换为字符 
			
			addPQueue(&huffmanQueue,aux,probability[i]);//传入出现次数，即优先级 
		}
